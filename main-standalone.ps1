# main-standalone.ps1
# Self-contained script including all configurations

$ErrorActionPreference = "Stop"

# Embedded configurations
$commentTypesScript = @'
# Embedded comment-types.ps1
return @{
    # [Content of comment-types.ps1 goes here]
    # This will be replaced during compilation
}
'@

$pathFiltersScript = @'
# Embedded path-filters.ps1
return @{
...
# Load embedded configurations
$fileTypes = Invoke-Expression $commentTypesScript
$pathFilters = Invoke-Expression $pathFiltersScript
Invoke-Expression $filterUtilsScript  # Load utility functions

if (-not $fileTypes) {
    Write-Error "Failed to load embedded comment types configuration"
    exit 1
}

if (-not $pathFilters) {
    Write-Error "Failed to load embedded path filters configuration"
    exit 1
